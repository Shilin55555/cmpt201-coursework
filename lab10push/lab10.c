...
The client uses TCP socket (AF_INET, SOCK_STREAM) to send data to the server.
2. What data is the client sending to the server?
Each client sends NUM_MSG = 5 text messages: "Hello", "Apple", "Car", "Green",
"Dog". Each message is written as a fixed-size buffer of BUF_SIZE bytes.

Understanding the Server:
1. Explain the argument that the `run_acceptor` thread is passed as an argument.
The argument is a pointer to struct acceptor_args. Which includes:
An atomic_bool run flag which is used to stop the acceptor loop,
A pointer to the shared list_handle (for the linked list of received messages),
A pointer to the shared list_lock (mutex protecting the list).
This allows the acceptor thread to share the list and mutex with all client
threads.
2. How are received messages stored?
Each received message is copied into dynamically memory and wrapped in
a struct list_node. These nodes form a singly-linked list whose handle is stored
in struct list_handle. New nodes are appended using add_to_list(), and the list
is protected by a mutex.
3. What does `main()` do with the received messages?
main() starts the acceptor thread and then waits until the total number of
received messages (list_handle.count) reaches MAX_CLIENTS * NUM_MSG_PER_CLIENT.
Then it stops the acceptor, joins it, calls collect_all() to print and free all
messages, and finally checks that all messages were collected.
4. How are threads used in this sample code?
There is one acceptor thread that runs accept() in a loop. For each new client
connection, a separate client thread is created. Each client thread reads from
its socket and pushes received messages into the shared linked list.
*/
#include <arpa/inet.h>
#include <stdio.h>
#include <stdlib.h>
...
The helper function set_non_blocking(fd) calls fcntl(fd, F_GETFL) and then
fcntl(fd, F_SETFL, flags | O_NONBLOCK).

What sockets are made non-blocking?
The main listening server socket (sfd) and each accepted client socket (cfd).

Why are these sockets made non-blocking? What purpose does it serve?
Non-blocking sockets prevent accept() and read() from blocking a thread
indefinitely. If there is no new connection or no data available, the call
returns immediately with EAGAIN/EWOULDBLOCK. This allows the acceptor and client
threads to keep running without getting stuck in a blocking system call.
*/
#include <arpa/inet.h>
#include <errno.h>
#include <fcntl.h>
